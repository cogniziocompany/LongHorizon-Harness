...
- 12:40 PT: PTAIT09 fully green: task Running, `Connected`, session file `svc/claude-rc.session.json` written (session_015FjKR9C2fTNqJX1oS9i8ij, heartbeat updatedAt ticking). Two more host bugs fixed + merged: #41 session ids carry underscores, #42 helper import needs file:// on Windows. easybutt0n-runner-v2 @ 55c667c. Next: e2e `npm run test:claude-rc`; then desk03 / htpc01 / ptait10am5 via their runners (install as the device's install user; runner is SYSTEM so use Register-ScheduledTask -UserId <user> -LogonType Interactive), CT hosts via compose profile; then cognizioware-qa fleet check.

## Follow-up 2026-09-07 22:30 PT: move Remote Control into Hydra (queue 13b)
Paxton: the RC host should not be hosted on PTAIT09; it should run through Hydra as a skill/tool for chat.easybutt0n.ai and Hydra's terminals, since Hydra is the control plane. Queue `13b-hydra-remote-control` (cognizioware-hydra-rsi, kimi, 8 rounds, text `C:\tmp\hydra-remote-control-task.txt`): design note, Hydra-managed RC host container on corsairai300, fleet-MCP tools rc.status/rc.sessions/rc.exec (approval-gated, audited, device allow-list) + a `remote-control` chat skill, migration script that enrols desk03/htpc01/ptait10am5 against the Hydra host and only then disables the PTAIT09 Scheduled Task (operator step; PTAIT09 never touched by the run). Ship Plane row updated to "moving to Hydra (13b)".
