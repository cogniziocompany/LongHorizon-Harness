...
- 21:25 PT: the merged qa-backend.compose.yml (PR #7) defines a NEW stack (postgres 15, new container names, secrets under /etc/cognizioware-qa/secrets, named volumes) rather than the running repo-* stack (postgres 17 with live data), so the "one-time" step is a data migration, not a recreate - deferred to a documented operator window. Interim protection applied on CT210: `docker update --restart unless-stopped repo-backend-1 repo-postgres-1`, so the backend survives the next daemon restart. The gate's /api/health path fix and infra-blocked reporting are live on master.

- 2026-09-07 23:25 PT: second powerplatform LAN runner `ct210-pp2` (labels self-hosted, lan-deploy) registered on CT210 as a systemd service, because the single `ct210-pp` runner serialised the hour-long eval gates with every dev deploy (Env→Session dev deploy sat queued 80 min behind the restore promotion). Both online. Eval gates should still not run two at once against the same dev app - watch for that; if it bites, pin eval-gate jobs to `ct210-pp` by label.
