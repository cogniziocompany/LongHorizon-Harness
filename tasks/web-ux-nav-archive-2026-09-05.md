...
## Deployment check (2026-09-07 17:45 PT, Paxton asked "have you completed this deployment")
- The nav/archive build (W1-W6, G1, migration 028) merged to develop as #67 and was promoted to prod in the pinned promotion at 476811f on 2026-09-06 (eval 100). BUT prod now runs sha-f59c833 = `main`: PR #79 (CI-only fix) merged to main afterwards and the push-to-main rollout redeployed prod from main, which is 92 commits behind the promoted sha. So the feature is NOT live on prod at this moment (dev has it; prod regressed silently). Fix in two parts: the in-flight promotion of develop 845bc89 (dispatched 17:20 PT) re-deploys prod with everything; the governance fix (main fast-forwarded by promote.yml; push-to-main must not roll prod with an unpromoted sha) is harness task 06d-pp-prod-promotion-branch-guard. G2 remains deferred; the Playwright spec still needs MSAL creds to run.
- 17:55 PT (Paxton): use cognizioware-qa's env files/ce-auth for the MSAL-gated Playwright checks. Dispatched the existing `web-ux` gate target against dev (run 34170669002) to see current coverage; queued 06e-qa-webux-nav-archive-flow to add a dedicated nav/archive flow (shell, breadcrumb, env-scoped sessions, archive/restore with no "Delete" copy, G1 status API) to that target, run against dev now and prod after the promotion lands.
- 18:50 PT: web-ux gate dispatches (34170962602, 34171112859) fail at "Wait for QA backend to be healthy" because that job polls /health (404) while the backend serves /api/health (the ce job already uses /api/health). QA backend + postgres restarted via compose on CT210 (they had been down since 11:40 UTC). Fix folded into the qa task, moved to the front as 05c; 06e (nav-archive flow) follows. Until then the nav-archive verification via the gate is blocked by that one-line path bug.
- 21:40 PT: with the QA backend fixed, the web-ux gate (run 34174385728) now runs the Playwright spec but fails in "Gate report (web-ux)": scripts/gate-report.mjs rejects `--source junit`. Folded into the nav-archive flow task as its first slice and moved up the queue as 05f (7 rounds).
