...
### INCIDENT 2026-09-07 ~03:15 PT: ptait01 hung after the swap zvol was added
Symptoms: ICMP replies, TCP accepts, but sshd sends no banner, pveproxy 8006 / CT202 + CT204 LiteLLM / span Ollama / pihole DNS (192.168.21.3) all silent. LAN DNS down -> GitHub unreachable from the PC. Prime suspect: swap on a ZFS zvol (`rpool/swap`, added 03:12 PT) under memory pressure - the known ZFS swap deadlock. No cluster, no IPMI path known; recovery needs a physical/IPMI reset by Paxton. FIRST action once reachable: `swapoff -a; sed -i '/rpool\/swap/d' /etc/fstab; rm -f /etc/sysctl.d/99-swap.conf; zfs destroy rpool/swap`. Do NOT use zvol swap again; if swap is wanted, use a swap file on a non-ZFS partition or add RAM. Launch queue + quota watcher stopped so nothing relaunches into the dead gateway.
- Recovery 2026-09-07 11:16: Paxton hard-reset ptait01; poller removed the zvol swap (fstab clean, dataset destroyed), host at 12/42 GB used. All CTs came back except CT204 (started by hand). pihole DNS answering again; CT202 + CT204 routers live. Both mcp-tools lanes had failed on the outage (E2E gate); lane 34105758274 (main = #87 + #89) re-run. Harness runs fd990c1c, 15fa88b8, b41549c8 resumed; launch queue, quota watcher, auto-deploy job and watchers restarted. Harness release deploy missed its idle window (clone failed on DNS) and waits again.

### INCIDENT 2026-09-07 ~13:40 PT: CT202 disk full -> prod LiteLLM lost its database
Cause chain: my 14 GB dump (kept as 3.3 GB gz) + repeated audioqa image builds (build cache 12.5 GB) filled the 40 GB CT202 disk to 100%. Postgres stops on ENOSPC; the router reported db: disconnected. While freeing space I ran `docker container prune -f`, which removed the STOPPED postgres container (data volume `cognizioware-mcp-tools-postgres-data` intact). Recreating it failed on a stale libnetwork endpoint ("already exists") that survived `network disconnect -f`; fixed by `systemctl restart docker` on CT202 (Paxton: APPROVE AUTO), then `docker compose --env-file mcp-tools.env up -d`. Result: postgres back (87 tables), router db connected, 46 containers up.
LESSONS: (1) never prune containers on a box that just hit ENOSPC without listing what stopped; (2) keep dumps OFF the CT disk (host /root/litellm-upgrade-ct202/); (3) image builds on the prod CT need a disk check first - the deploy job should `df` and prune build cache before `docker build`; (4) a corrupted docker network endpoint after ENOSPC needs a daemon restart.
